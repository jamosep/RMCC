...
// Verificar si se recibieron los parámetros del token y username
if (isset($_GET['token']) && isset($_GET['username'])) {
    // Establecer la sesión PHP
    $_SESSION['username'] = $_GET['username'];
    $_SESSION['token'] = $_GET['token'];
    
    // Redirigir a la misma página sin los parámetros en la URL
...
if (!isset($_SESSION['username'])) {
    // Si el usuario no ha iniciado sesión, redirigir a login.jsp
    header("Location: http://localhost:8080/RMCCProject/login.jsp");
    exit();
}
?>
